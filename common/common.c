...
{
	if ( USART_GetFlagStatus(DBG_COM, USART_FLAG_RXNE) != RESET)
	{
		*key = (uint8_t)DBG_COM->DR;
		return 1;
	}
	else
	{
...
{
	USART_SendData(DBG_COM, c);
	while (USART_GetFlagStatus(DBG_COM, USART_FLAG_TXE) == RESET)
	{
	}
}
...
void Main_Menu(void)
{
  ...
  while (1)
  {
    ...
    SerialPutString("  Upload Image From Flash ------- 2\r\n\n");
    SerialPutString("  Execute ------------------------------ 3\r\n\n");
